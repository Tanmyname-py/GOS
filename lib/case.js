...
import pkg from "@vreden/youtube_scraper";
const { search, ytmp3, ytmp4, ytdlv2, channel } = pkg;
  ...
  try {
    ...
    switch (command) {
      ...
      case 'ytmp3': {
        ...
        await Loading()
        let result = await ytmp3(text, "128");
        if (result.status) {
          await sendAudio(result?.download.url, result?.metadata.title);
        } else {
      ...
      case 'ytmp4': {
        if (!text) return reply('Input url from YouTube ')
        await Loading()
        let result = await ytmp4(text, "720")
        console.log(result);
      ...
      case 'play': {
        ...
        let results = await ytmp3(url, "128");
        if (results.status) {
          await sendAudio(results?.download.url, results?.metadata?.url);
        } else {
          reply('Download error');
        }
        break
      ...
      case 'sticker': {
        ...
        //Get keys group with method object 
        Object.keys(group).map((key) => {
          reply(`Group: ${group[key]?.subject}\nIdgc : ${group[key]?.id}\nDesc Group : ${group[key]?.desc || ''}\nMember count : ${group[key].participants.length}  `)
        })
        break
      }
      case 'ping':
...
`.trim();
      ...
      case 'tagall': {
        ...
        let members = metadata?.participants.map(member => {
          return `🍀 @${member?.id.replace('@s.whatsapp.net', '')}`
        })
        let info = `---------------------------------------------
          🍀 *TAG ALL* 🍀
${members.join('\n')}
---------------------------------------------`
      ...
      default: {
        reply("uknwon command not available");
      }
    }
  } catch (error) {
    console.log(error)
