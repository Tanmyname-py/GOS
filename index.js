...
require('./gos.js')
nocache('../gos.js', module => console.log(color('[ CHANGE ]', 'green'), color(`'${module}'`, 'green'), 'Updated'))
require('./index.js')
...
async function Tan2() {
	...
	const msgRetryCounterCache = new NodeCache()
     ...
     auth: {
      ...
      browser: [ 'Linux', 'Chrome', '91.0.4472.124' ], // for this issues https://github.com/WhiskeySockets/Baileys/issues/328
...
if (pairingCode && !Tan.authState.creds.registered) {
    ...
    while (true) {
        ...
        if (phoneNumberObj && phoneNumberObj.isValid()) {
            phoneNumber = cleanedNumber; // Use the cleaned number for further processing
            break; // Valid phone number, exit loop
        } else {
            console.log(chalk.bgBlack(chalk.redBright("Nomor yang dimasukkan tidak valid. Pastikan nomor dimulai dengan kode negara, contoh: +6281328139682.")));
        }
    }
    ...
    setTimeout(async () => {
        try {
            ...
            console.error(chalk.bgRed(chalk.white("Error requesting pairing code:")), error);
        }
    }, 3000);
}
...
require('./gos.js')(Tan, m, chatUpdate, store)
} catch (err) {
console.log(err)}})

	// detect group update
// Deteksi update pada peserta grup
Tan.ev.on('group-participants.update', async (anu) => {
    if (global.wlcm) console.log(anu);
    try {
        let metadata = await Tan.groupMetadata(anu.id);
        const groupDesc = metadata.desc; // Mengambil deskripsi grup
        ...
        for (let num of participants) {
            ...
            if (anu.action == 'add') {
                // Mengirim pesan sambutan dengan foto grup dan deskripsi grup
                let welcomeText = `WELCOME👋 @${num.split("@")[0]} To ${metadata.subject}\n${groupDesc}`;
                ...
                // Mengirim pesan perpisahan dengan foto grup
...
const resolveBuffer = async (path) => {
  ...
  throw new Error('Invalid path or URL provided');
};
Tan.sendImageAsSticker = async (jid, path, quoted, options = {}) => {
  let buffer = await resolveBuffer(path);
  if (options.packname || options.author) {
    buffer = await writeExifImg(buffer, options);
  } else {
...
Tan.sendVideoAsSticker = async (jid, path, quoted, options = {}) => {
  let buffer = await resolveBuffer(path);
  if (options.packname || options.author) {
    buffer = await writeExifVid(buffer, options);
  } else {
...
if (message.message && message.message.viewOnceMessage && message.message.viewOnceMessage.message) {
    vtype = Object.keys(message.message.viewOnceMessage.message)[0];
} else {
    console.log("viewOnceMessage.message is undefined or null");
}
